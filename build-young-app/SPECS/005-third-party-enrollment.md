...
3. The student is **attributed to that partner** where it matters: the funnel `enrolled` event
   carries `source:"partner:<id>"` (aggregate, no PII); the cohort roster marks partner seats.
4. **Settlement (per partner):** the console shows, per partner — **seats**, **gross** (Σ list
   ...
   and **outstanding balance** (net owed − received). The founder **records a payment received**
   (date + amount + optional note); the balance updates. This is the ledger the founder uses to
   chase/track partner payments.
...
- [ ] The `enrolled` funnel event records the partner source (aggregate, no PII).
- [ ] The console shows a **per-partner settlement view** — seats, gross, partner cut, **net owed**,
      **received**, **outstanding balance** — and the founder can **record payments received**; the
      outstanding balance updates. Withdrawn partner seats are not counted as owed.
...
- **Touches:** founder console (partner **config** editor + partner-enroll action + **settlement**
  ...
  + snapshotted `priceCents` + `cutPct`), funnel (`source` tag + server allowlist), Platform
...
- **Funnel revenue interaction.** A partner seat realizes **net**, not list price. Decide whether
  partner seats count at net in the funnel `revenue` number or are excluded from it — the per-partner
  settlement view is the source of truth for partner money either way.
