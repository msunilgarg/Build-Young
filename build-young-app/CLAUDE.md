...
- **Refund policy (in code):** full refund before the cohort starts (state flag `started:false`, flips true on first `doAdvance`); prorated refund through the **first 3 weeks** = `price×(12−week)/12`; non-refundable after (shown explicitly to the student once Week 3 passes). NOTE: copy must say "first 3 weeks", NOT "Act 1" — after the build-first flip, Act 1 is the 6-week build arc, so the old "Act 1 = first 3 weeks" label is wrong. Logic lives in `Platform` (`notStarted`, `canWithdraw`, `refund`) and the Terms copy in `LEGAL`.
