...
- **`HeroPreview`** — animated dashboard mock on the landing page; cycles the three acts: Build (Wk3) → Grow (Wk8) → Capstone (Wk12, a certificate + "presenting live").
...
- **The Agentic Engineering Process (SPECS/008) — the named through-line:** the build method the course teaches (and that Build Young is itself built with) is **Spec → Build → Check → Ship**, surfaced as `AgenticProcessPrimer` (Platform.jsx) — full card at the head of Lesson 2 (`ShapePlan`, where the method kicks in / the spec is written), a compact reminder strip at the head of Act 2 (Lesson 8) and Act 3 (Lesson 11). Steps: **Spec** (write the "done-when" first), **Build** (one slice), **Check** (the Lesson-3–6/8 "Check my work" independent-AI review, SPECS/008 T39/T40), **Ship** (Lesson 7 Go Live). Copy is POSITIONING-voiced (build with AI, not coding; us/we). Deferred follow-ons: a per-student project-brief doc + a "how Build Young is built" showcase. **The Check agent (T39):** `api/_lib/reviewAgent.js` (mirrors `scenarioAgent.js`) grades a student's build against their `s.shape.acceptance` → `{verdict:"pass"|"gaps", strengths[], gaps[]}` (sanitized); called via **public `POST /api/funnel?resource=review`** using **Build Young's own `ANTHROPIC_API_KEY`** (server-side; students bring none). Founder-toggleable via **separate** ops `reviewAgentEnabled` + `reviewModel` (`ReviewAgentEditor` in the console; `saveOps` merges so it won't clobber `notifyEmail`/scenario settings); off / no key ⇒ a free deterministic `localReview` self-check so the step always works. **Client (T40):** `BuildLayer` (Lessons 3–6 + 8) renders the "Check my work" card (paste-what-you-built → POST → strengths-first/gaps/verdict), result saved to **`s.review[week]`**; it's optional and **never gates progression**, and a full outage falls back to `localReview` client-side (it imports the pure helper) so it never errors. The card also exposes the **`s.shape.acceptance` criteria as editable in the build week** (same SoT as Lesson 2 — refine as you learn; edits sync back). **Framing (T41):** Lesson 7 `GoLiveChecklist` names itself the **"Ship"** step of the loop (copy only — checklist mechanics unchanged), and Lesson 11 capstone-prep (`REFLECT_WEEKS[11].intro`) tells the build story as Spec → Build → Check → Ship. **Project kit (SPECS/009 — in progress):** `src/projectKit.js` (foundation, pure) is the **single source of `AGENTIC_STEPS`** (imported by `AgenticProcessPrimer`) and `buildProjectKit({build, shape})` compiles `s.build`+`s.shape` → the student's **`CLAUDE.md`/`SPEC.md`/`POSITIONING.md`/`PLAYBOOK.md`** (the docs their build-AI reads every session, with guardrails baked in) — the deterministic base. **Lesson-2 UI (T43):** `ProjectKitPanel` (in `ShapePlan`) renders a **"Set up with Claude Code"** button (copies one prompt that has the student's *own* agent write all four files) + per-file download + a readonly setup-prompt preview (copy fallback); always reflects the live spec (re-generatable). T42/T43 done. **Create-your-repo (T44):** a `repo` item in `PREREQS` explains in plain English what a repo is + (when set) a one-click **"Use this template"** link to the founder's starter template repo — driven by a new founder-editable site setting **`starterRepoUrl`** (`src/site.js`; hidden when blank). Rendered via a generic `configLink` field on prereq items. (Known pre-existing bug, separate: `prereqWeek`'s `/week N/` regex doesn't match the `"Lesson N"` `when` values, so the per-lesson Pre-req TAB shows the generic note instead of the tool checklist — the repo step still shows on the main "Get set up" panel.) **Optional AI polish (T45):** `api/_lib/kitAgent.js` (mirrors `reviewAgent.js`; reuses `buildProjectKit` + `parseJsonObject`) sharpens the four files while keeping the filenames/guardrails/acceptance contract — `sanitizeKit` returns EXACTLY the four files, capped, **falling back to the deterministic base per-file** so a bad reply can't break the kit. Public **`POST /api/funnel?resource=kit`** → `{configured, kit}` (off / no key / any failure ⇒ the deterministic `buildProjectKit` kit). Founder ops `kitAgentEnabled` + `kitModel` (`KitAgentEditor` console; `saveOps` merges). Client: `ProjectKitPanel`'s **"Polish with AI"** button swaps in the polished kit when configured; a spec edit invalidates a prior polish (re-generatable). **Spec 009 Phase 1 (T42–T45) complete** — Path C (GitHub auto-commit for minors) stays deferred to a consent/auth phase.
