...
- **The Agentic Engineering Process (SPECS/008) — the named through-line:** the build method the course teaches (and that Build Young is itself built with) is **Spec → Build → Check → Ship**, surfaced as `AgenticProcessPrimer` (Platform.jsx) — full card at the head of Lesson 2 (`ShapePlan`, where the method kicks in / the spec is written), a compact reminder strip at the head of Act 2 (Lesson 8) and Act 3 (Lesson 11). Steps: **Spec** (write the "done-when" first), **Build** (one slice), **Check** (the Lesson-2–6/8 "Check my work" independent-AI review, SPECS/008 T39/T40), **Ship** (Lesson 7 Go Live). Copy is POSITIONING-voiced (build with AI, not coding; us/we). Deferred follow-ons: a per-student project-brief doc + a "how Build Young is built" showcase. **The Check agent (T39):** `api/_lib/reviewAgent.js` (mirrors `scenarioAgent.js`) grades a student's build against that feature's `s.shape.accept[key]` (per-feature "Done when…", SPECS/012) → `{verdict:"pass"|"gaps", strengths[], gaps[]}` (sanitized); called via **public `POST /api/funnel?resource=review`** using **Build Young's own `ANTHROPIC_API_KEY`** (server-side; students bring none). Founder-toggleable via **separate** ops `reviewAgentEnabled` + `reviewModel` (`ReviewAgentEditor` in the console; `saveOps` merges so it won't clobber `notifyEmail`/scenario settings); off / no key ⇒ a free deterministic `localReview` self-check so the step always works. **Client (T40):** `BuildLayer` (Lessons 2–6 + 8) renders the "Check my work" card (paste-what-you-built → POST → strengths-first/gaps/verdict), result saved to **`s.review[week]`**; it's optional and **never gates progression**, and a full outage falls back to `localReview` client-side (it imports the pure helper) so it never errors. The criteria are written as **step ② (`s.shape.accept[key]`, per-feature — editable in the build week**; refine as you learn). **Framing (T41):** Lesson 7 `GoLiveChecklist` names itself the **"Ship"** step of the loop (copy only — checklist mechanics unchanged), and Lesson 11 capstone-prep (`REFLECT_WEEKS[11].intro`) tells the build story as Spec → Build → Check → Ship. **Project kit (SPECS/009 — in progress):** `src/projectKit.js` (foundation, pure) is the **single source of `AGENTIC_STEPS`** (imported by `AgenticProcessPrimer`) and `buildProjectKit({build, shape})` compiles `s.build`+`s.shape` → the student's **`CLAUDE.md` + a `SPECS/` folder (one spec per feature + an overview, SPECS/011) + `POSITIONING.md` + `PLAYBOOK.md`** (the docs their build-AI reads every session, with guardrails baked in) — the deterministic base. **Lesson-2 UI (T43; collapsed after the spec per SPECS/012):** `ProjectKitPanel` renders a **"Set up with Claude Code"** button (copies one prompt that has the student's *own* agent write all the files) + per-file download + a readonly setup-prompt preview (copy fallback); always reflects the live spec (re-generatable). T42/T43 done. **Create-your-repo (T44):** a `repo` item in `PREREQS` explains in plain English what a repo is + (when set) a one-click **"Use this template"** link to the founder's starter template repo — driven by a new founder-editable site setting **`starterRepoUrl`** (`src/site.js`; hidden when blank). Rendered via a generic `configLink` field on prereq items. (Fixed: `prereqWeek`'s regex now matches `"Lesson N"` — previously `/week N/` matched nothing, so the per-lesson Pre-req tab showed a "come in Week Infinity" artifact instead of the tool checklist.) **Optional AI polish (T45):** `api/_lib/kitAgent.js` (mirrors `reviewAgent.js`; reuses `buildProjectKit` + `parseJsonObject`) sharpens the files while keeping the filenames/guardrails/acceptance contract — `sanitizeKit` returns EXACTLY the kit files, capped, **falling back to the deterministic base per-file** so a bad reply can't break the kit. Public **`POST /api/funnel?resource=kit`** → `{configured, kit}` (off / no key / any failure ⇒ the deterministic `buildProjectKit` kit). Founder ops `kitAgentEnabled` + `kitModel` (`KitAgentEditor` console; `saveOps` merges). Client: `ProjectKitPanel`'s **"Polish with AI"** button swaps in the polished kit when configured; a spec edit invalidates a prior polish (re-generatable). **Spec 009 Phase 1 (T42–T45) complete** — Path C (GitHub auto-commit for minors) stays deferred to a consent/auth phase.
...
- **Curriculum structure (WEEKS) — BUILD-FIRST, THREE acts, 12 weeks (7/3/2):** founder's outline:
  ...
  **One spec per build week (SPECS/011) — a consistent 3-step loop (SPECS/012):** every build week,
  `BuildLayer` shows the SAME three labeled steps, each with a file-mapping pill showing the SPECS file it
  writes: **① Write your spec** (`s.shape[key]`) → **② Write your acceptance criteria** — that feature's own
  **"Done when…"** (`s.shape.accept[key]`) → **③ Check my work — the verifier** (paste-what-you-built →
  independent review vs THIS feature's `accept[key]`). The "Copy" in step ① hands Claude a *"create
  `SPECS/<feature>.md` (spec **+ its Done-when**) → commit → build"* handoff (`specFileFor` maps key→filename).
  `s.shape` holds one field per feature — `product`(Wk2)/`accounts`(Wk3)/`payments`(Wk4)/`production`(Wk5)/
  `polish`(Wk6)/`funnel`(Wk8) — plus `success` (the product vision → `SPECS/000-overview.md`) and **`accept`**,
  an object of **per-feature** "Done when…" criteria (`accept[key]`, each folded into that feature's own
  `SPECS/<feature>.md` — SPECS/012 reversed SPECS/011's single global `acceptance`). `BUILD_LAYERS` (keyed by
  ...
  truth). **Lesson 2 is the first build week:** `ShapePlan` (the Agentic-Process primer + product vision
  `success` ONLY) → the core-product `BuildLayer` (the 3 steps) → the **project kit** rendered **collapsed,
  AFTER the spec** (`ProjectKitPanel collapsed` — `buildProjectKit` → CLAUDE.md + a `SPECS/` folder, one file
  per feature (spec + its Done-when) + an overview, + POSITIONING.md + PLAYBOOK.md), so the files reflect the
  spec just written. **Week 8** (`funnel`) is specced in-week against a worked
