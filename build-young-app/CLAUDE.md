...
- **Curriculum structure (WEEKS) — BUILD-FIRST, THREE acts, 12 weeks (7/3/2):** founder's outline:
  ...
  **2, 3, 4, 5, 6, 8**; `BUILD_WEEKS` = that set) drives `BuildLayer`, which shows ONLY that week's spec
  field + the commit-&-build handoff. **The whole plan is ONE object — `s.shape`**: every build week
  reads/writes its own `s.shape[key]` (single source of truth). **Lesson 2 is the first build week:**
  `ShapePlan` (the Agentic-Process primer + product vision `success` + the **project kit** —
  `buildProjectKit` → CLAUDE.md + a `SPECS/` folder, one file per feature + an overview, + POSITIONING.md +
  PLAYBOOK.md) then the core-product `BuildLayer`. **Week 8** (`funnel`) is specced in-week against a worked
