...
- **Curriculum structure (WEEKS) — BUILD-FIRST, THREE acts, 12 weeks (7/3/2):** founder's outline:
  ...
  **① Write your spec** (`s.shape[key]`) → **② Write your acceptance criteria** (`s.shape.accept[key]`,
  ...
  + the `SPECS/` folder, one file per feature with its acceptance criteria + an overview index + POSITIONING.md
  ...
  **`accept`**, an object of **per-feature** acceptance criteria (`accept[key]`, each folded into that feature's
  ...
  it + the `success` field — product metrics are their own lesson, Wk9). **NOTE (SPECS/013 follow-up):** the
  server `reviewAgent` + `ReviewAgentEditor` console + `reviewAgentEnabled`/`reviewModel` ops are now unused by
  the student flow (kept inert; a separate cleanup PR removes them). **Week 8** (`funnel`) is specced in-week against a worked
