...
- **Curriculum structure (WEEKS) — BUILD-FIRST, THREE acts, 12 weeks (7/3/2):** founder's outline:
  ...
  **"Done when…"** (`s.shape.accept[key]`) → **③ Check my work — the verifier** (paste-what-you-built →
  ...
  `polish`(Wk6)/`funnel`(Wk8) — plus **`accept`**, an object of **per-feature** "Done when…" criteria
  (`accept[key]`, each folded into that feature's own `SPECS/<feature>.md` — SPECS/012 reversed SPECS/011's
  single global `acceptance`). `BUILD_LAYERS` (keyed by weeks **2, 3, 4, 5, 6, 8**; `BUILD_WEEKS` = that set)
  drives `BuildLayer`. **The whole plan is ONE object — `s.shape`**: every build week reads/writes its own
  `s.shape[key]` + `s.shape.accept[key]` (single source of truth). Each build week's `BuildLayer` is a clean,
  consistent loop — **① spec → ② acceptance → ③ check** — every box just heading + a file-mapping badge + the
  field (SPECS/012; no callouts/captions). **Lesson 2 is the first build week:** the `AgenticProcessPrimer`
  (method intro, rendered directly) → the core-product `BuildLayer`, where the **project kit** is a
  **REQUIRED step between ② acceptance and ③ check** (passed in as `BuildLayer`'s `beforeCheck` prop —
  `ProjectKitPanel`, not collapsed: `buildProjectKit` → CLAUDE.md + a `SPECS/` folder, one file per feature
  (spec + its Done-when) + an overview index, + POSITIONING.md + PLAYBOOK.md), so you set up the docs your AI
  reads, then build, then check. **There is NO standalone product-vision panel** (SPECS/012 removed it + the
  `success` field — product metrics are their own lesson, Wk9). **Week 8** (`funnel`) is specced in-week against a worked
