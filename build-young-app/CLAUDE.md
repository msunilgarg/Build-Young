...
- **The Agentic Engineering Process (SPECS/008) — the named through-line:** the build method the course teaches (and that Build Young is itself built with) is **Spec → Build → Check → Ship**, surfaced as `AgenticProcessPrimer` (Platform.jsx) — full card at the head of Lesson 1 (`BuildPlan`), a compact reminder strip at the head of Act 2 (Lesson 8) and Act 3 (Lesson 11). Steps: **Spec** (write the "done-when" first), **Build** (one slice), **Check** (the Lesson-3–6/8 "Check my work" independent-AI review, SPECS/008 T39/T40), **Ship** (Lesson 7 Go Live). Copy is POSITIONING-voiced (build with AI, not coding; us/we). Deferred follow-ons: a per-student project-brief doc + a "how Build Young is built" showcase. **The Check agent (T39):** `api/_lib/reviewAgent.js` (mirrors `scenarioAgent.js`) grades a student's build against their `s.shape.acceptance` → `{verdict:"pass"|"gaps", strengths[], gaps[]}` (sanitized); called via **public `POST /api/funnel?resource=review`** using **Build Young's own `ANTHROPIC_API_KEY`** (server-side; students bring none). Founder-toggleable via **separate** ops `reviewAgentEnabled` + `reviewModel` (`ReviewAgentEditor` in the console; `saveOps` merges so it won't clobber `notifyEmail`/scenario settings); off / no key ⇒ a free deterministic `localReview` self-check so the step always works. **Client (T40):** `BuildLayer` (Lessons 3–6 + 8) renders the "Check my work" card (paste-what-you-built → POST → strengths-first/gaps/verdict), result saved to **`s.review[week]`**; it's optional and **never gates progression**, and a full outage falls back to `localReview` client-side (it imports the pure helper) so it never errors. **Framing (T41):** Lesson 7 `GoLiveChecklist` names itself the **"Ship"** step of the loop (copy only — checklist mechanics unchanged), and Lesson 11 capstone-prep (`REFLECT_WEEKS[11].intro`) tells the build story as Spec → Build → Check → Ship.
