...
<!-- ===== Spec 024 — location on enrollments & scholarship applications. See SPECS/024-location-on-enrollments-and-applications.md. SHIPPED (T50). ===== -->

<!-- Completed tasks are checked off and moved below this line by the loop, newest first. -->
## Done

## [x] T50 — Location (country + US state) on enrollments & scholarship applications  ·  risk: med
Done (SPECS/024). Server-stamp country (+ US state) from Vercel geo headers via `geoFromReq(req)` (`api/funnel.js`) onto `visited`, `enrolled` (in `ingest` — a direct paid enroll fires from the student's browser), and the scholarship `free_application` (in `addFreeApplication`; also persisted on the stored record via `addEnrollment` country/region → surfaced by `listFreeEnrollments`). Server-derived, not in `ALLOWED_PROPS` (unspoofable), country/state only (no city/IP). NOT stamped on server-fired `enrolled` (scholarship award / partner onboard — headers are the founder's), so scholarship location lives on the application; partner seats are location-less. Display: new pure `enrollmentGeography(events)` (`src/funnel.js`) → a period-scoped "Where enrollments & applications come from" card (country + US-state drill-down) in the Funnel tab; per-applicant `country (region)` on each Students → Free applications row. Going-forward only (no backfill). Tests: enrollmentGeography (tally/empty), free-enroll geo on event+record + non-US + ingest-enrolled-with-spoof-ignored, founder-ui card + list location. CLAUDE.md geo note. Build + 461; independently verified.
