...
|---|---|---|
| **State file** | survives between runs so the loop has memory | `TASKS.md` (backlog + progress), `CLAUDE.md` (project knowledge — auto-loaded, and `@import`s `ENGINEERING-PLAYBOOK.md`), `POSITIONING.md` (copy & voice source of truth), `LOOP.md` (this manual) — all committed, so they survive ephemeral container resets. (See the "Governing docs" cluster in `BUILD-YOUNG-ARCHITECTURE.md` for which are auto-loaded vs read on demand.) |
