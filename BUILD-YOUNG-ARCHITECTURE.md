...
| **api/funnel.js** | One method-routed endpoint (Hobby 12-function cap): **POST** public event ingest (+ founder-gated `?resource=partner-enroll` → create a PENDING partner enrollment, inert), **GET** founder funnel read (+ `?resource=partners` / `?resource=partner-enrollments`), **PUT** saves cohorts/allowlist/settings/**partners**, **DELETE** resets a test account. Non-POST (and the founder-gated POST resources) require a founder session. |
