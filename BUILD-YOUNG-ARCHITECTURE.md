...
| **api/_lib/\*** | Server internals: `kv` (Vercel KV client), `auth`, the KV-backed stores (incl. `refundStore` pending refunds + `paymentIssueStore` failed-payment alerts), `schedule`, `resendAudience`, and the two student AI agents — `scenarioAgent` (Week-9 practice funnels) + `reviewAgent` (the build-weeks "Check my work" review, SPECS/008). Both call the Anthropic API with Build Young's own server-side key, are founder-toggleable, and fall back to a free local result when off/no key. |
