...
|---|---|
| **Triggers** | Two on-ramps to the same driver — **use one OR the other for a given task, never both.** **Local `/run-loop`** (runs in your Claude Code on your subscription; drains the `TASKS.md` backlog) **or** the **issue-triggered GitHub Action** (`.github/workflows/run-loop.yml`, gated by the `loop-task` label, billed to Anthropic API credits; the **issue itself is the task** — it doesn't read `TASKS.md`). Same procedure once started. **The spec-first gate is trigger-independent:** "the issue IS the task" is the right path for a **bug-level** issue, but a **non-trivial / feature** issue should be turned into a 📐 spec first (and promoted to a task) rather than run straight from a one-line issue — the trigger chooses *where the task comes from*, not *whether it earns a spec*. |
...
| **Foundation** | Shared, dependency-light single-sources-of-truth — imported by everything, so changes are **additive-only** during parallel work: `funnel.js` (stage/conversion/revenue math + traffic geography — country & US-state), `cohorts.js` (`SEASONS`/`BATCHES`), `course*.js`/`engine.js` (curriculum + week progression), `projectKit.js` (SoT for `AGENTIC_STEPS` + `FEATURE_SPECS`/`specFileFor`; compiles a student's per-feature specs → their `CLAUDE.md` + a `SPECS/` folder (one file per feature + overview) + `PITCH.md` (their living positioning — written Wk1, refined every build week) + `PLAYBOOK.md` (the loop + fixed security non-negotiables + a student-authored "Engineering rules" section, `s.build.rules`, refined every build week — SPECS/021) kit — SPECS/009 + 011 + 019 + 021), `founderStory.js` (compiles a student's capstone reflection → an honest application-ready founder one-pager — SPECS/010), `theme/ui/lib/site/cert/scenarios/marketMedia`. |
| **api/funnel.js** | One method-routed endpoint (Hobby 12-function cap): **POST** public event ingest (+ founder-gated `?resource=partner-enroll` → create a PENDING partner enrollment, inert · `?resource=partner-onboard` → "Start onboarding": same welcome email + account + audience + `enrolled` at net · `?resource=partner-remove` → founder removal: drop enrollment/audience/account, no refund · public `?resource=partner-lead` → "Partner with us" inquiry, stored + emailed · public `?resource=free-enroll` → free/scholarship application w/ write-up (PENDING, notifies founder) · founder `?resource=free-approve` → onboard at $0 (account + welcome email + audience + `enrolled` source:"free") · founder `?resource=free-remove` → decline/remove, SPECS/016), **GET** founder funnel read (+ `?resource=partners` / `?resource=partner-enrollments` / `?resource=free-enrollments` / `?resource=partner-lead` / `?resource=payment-failures`), **PUT** saves cohorts/allowlist/settings/**partners**, **DELETE** resets a test account. Non-POST (and the founder-gated POST resources) require a founder session. |
| **api/cohorts.js** | Public read of the live catalog (`batches`, `checkins`, `settings`, + featured **`partners`** display fields) so clients hydrate cohorts + site settings without a redeploy. |
| **api/state.js · auth/\*** | Student course state; account auth (login/logout/me/reset/set-password) — founder gating via `FOUNDER_EMAILS`. |
| **api/stripe-webhook.js** | Enrollment lifecycle: `checkout.session.completed` adds the student (+ Resend audience); `charge.refunded` removes enrollment + audience contact; **`payment_intent.payment_failed` / `charge.failed` / `checkout.session.async_payment_failed`** record a failed payment + email the founder via `paymentIssueStore` (visibility only — no charge/refund/enrollment change; idempotent per attempt so the dual events alert once). |
...
| **api/_lib/\*** | Server internals: `kv` (Vercel KV client), `auth`, the KV-backed stores (incl. `refundStore` pending refunds + `paymentIssueStore` failed-payment alerts), `schedule`, `resendAudience`, and the student AI agent — `scenarioAgent` (Week-9 practice funnels; calls the Anthropic API with Build Young's own server-side key, founder-toggleable, falls back to a deterministic result when off/no key). *(SPECS/014 removed the `reviewAgent` + `kitAgent`: build & verify now run in the student's own Claude, SPECS/013.)* |
