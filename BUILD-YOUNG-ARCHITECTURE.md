...
| **api/_lib/\*** | Server internals: `kv` (Vercel KV client), `auth`, the KV-backed stores (incl. `refundStore` pending refunds + `paymentIssueStore` failed-payment alerts), `schedule`, `resendAudience`, and `scenarioAgent` (calls the Anthropic API to generate Week-9 practice funnels; key stays server-side, founder-toggleable). |
