...
| **Foundation** | Shared, dependency-light single-sources-of-truth — imported by everything, so changes are **additive-only** during parallel work: `funnel.js` (stage/conversion/revenue math + traffic geography — country & US-state), `cohorts.js` (`SEASONS`/`BATCHES`), `course*.js`/`engine.js` (curriculum + week progression), `projectKit.js` (SoT for `AGENTIC_STEPS` + `FEATURE_SPECS`/`specFileFor`; compiles a student's per-feature specs → their `CLAUDE.md` + a `SPECS/` folder (one file per feature + overview) + `POSITIONING.md` + `PLAYBOOK.md` kit — SPECS/009 + 011), `founderStory.js` (compiles a student's capstone reflection → an honest application-ready founder one-pager — SPECS/010), `theme/ui/lib/site/cert/scenarios/marketMedia`. |
