...
| **api/_lib/\*** | Server internals: `kv` (Vercel KV client), `auth`, the KV-backed stores (incl. `refundStore` pending refunds + `paymentIssueStore` failed-payment alerts), `schedule`, `resendAudience`, and the student AI agents — `scenarioAgent` (Week-9 practice funnels) + `reviewAgent` (the build-weeks "Check my work" review, SPECS/008) + `kitAgent` (optional polish of the Lesson-2 project kit, SPECS/009). All call the Anthropic API with Build Young's own server-side key, are founder-toggleable (separate model per agent), and fall back to a free local/deterministic result when off/no key. |
